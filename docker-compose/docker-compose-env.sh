...
echo -e "${magenta} Make sure you have at least docker-compose==1.25.3 ${NC}"
# shellcheck disable=SC2154
echo -e "${green} pip install docker-compose==1.25.3 ${NC}"
...
if [ -n "${DOCKER_COMPOSE_UP_SERVICE}" ]; then
  echo -e "${green} DOCKER_COMPOSE_UP_SERVICE is defined ${happy_smiley} : ${DOCKER_COMPOSE_UP_SERVICE} ${NC}"
else
  echo -e "${red} ${double_arrow} Undefined build parameter ${head_skull} : DOCKER_COMPOSE_UP_SERVICE, use the default one ${NC}"
  export DOCKER_COMPOSE_UP_SERVICE="web"
  echo -e "${magenta} DOCKER_COMPOSE_UP_SERVICE : ${DOCKER_COMPOSE_UP_SERVICE} ${NC}"
fi
...
if [ -n "${DOCKER_COMPOSE_UP_OPTIONS}" ]; then
  ...
  export DOCKER_COMPOSE_UP_OPTIONS="--force-recreate -d ${DOCKER_COMPOSE_UP_SERVICE}"
