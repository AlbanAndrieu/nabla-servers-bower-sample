...
echo -e "${magenta} time docker build ${DOCKER_BUILD_ARGS} -f ${WORKING_DIR}/docker/ubuntu16/Dockerfile -t \"${DOCKER_ORGANISATION}/${DOCKER_NAME}\" \"${WORKING_DIR}/..\" --tag \"$DOCKER_TAG\" ${NC}"
time docker build ${DOCKER_BUILD_ARGS} -f "${WORKING_DIR}/../docker/ubuntu16/Dockerfile" -t "${DOCKER_ORGANISATION}/${DOCKER_NAME}" "${WORKING_DIR}/.." --tag "${DOCKER_TAG}"
RC=$?
if [ ${RC} -ne 0 ]; then
  echo ""
...
echo -e "docker build -t ${DOCKER_ORGANISATION}/${DOCKER_NAME}:latest --pull -f ${WORKING_DIR}/../docker/ubuntu16/Dockerfile ./"
